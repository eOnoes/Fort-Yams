...
import {
  ...
  executeAgentsOutbox,
  executeAgentsRead,
  executeAgentsCreateTask,
  executeAgentsArchive,
  executeAgentsReject,
  executeAgentsReview,
  executeAgentsReviews,
  executeAgentsReviewRead,
} from "../agentsCommand.js";
...
describe("tripp agents read", () => {
  ...
  it("rejects path traversal", async () => {
    await expect(
      executeAgentsRead("/etc/passwd", tmpDir)
    ).rejects.toThrow(/traversal/);
  });

  it("rejects paths outside Agent Bus", async () => {
    await expect(
      executeAgentsRead("/tmp/outside.json", tmpDir)
    ).rejects.toThrow(/traversal/);
  });
});
...
async function createResultPacket(
  opts: {
    packetId?: string;
    runId?: string;
    agentRole?: string;
  ...
  const {
    writeResultPacket: writeResult,
    SCHEMA_VERSION,
  } = await import("@tripp-reason/external-agents");
...
describe("tripp agents review", () => {
  it("creates valid Echo review packet and report from result", async () => {
    ...
    const mdContent = await fs.readFile(
      path.join(reportsDir, mdFiles[0]),
      "utf-8"
    );
  ...
  it("rejects review of result outside Agent Bus", async () => {
    await expect(
      executeAgentsReview("/tmp/outside.json", {
        verdict: "pass",
        summary: "nope",
        workdir: tmpDir,
      })
  ...
  it("rejects block verdict without issue or safety finding", async () => {
    ...
    await expect(
      executeAgentsReview(resultPath, {
        verdict: "block",
  ...
  it("review report includes approval boundary warning", async () => {
    ...
    const reportsDir = path.join(tmpDir, ".tripp/agents/reports");
    const mdFiles = (await fs.readdir(reportsDir)).filter((f) =>
      f.endsWith(".md")
    );
    const mdContent = await fs.readFile(
      path.join(reportsDir, mdFiles[0]),
  ...
  it("mutation-bearing result review includes ApprovalGate reminder", async () => {
    ...
    const reportsDir = path.join(tmpDir, ".tripp/agents/reports");
    const jsonFiles = (await fs.readdir(reportsDir)).filter((f) =>
      f.endsWith(".json")
    );
    const jsonContent = await fs.readFile(
      path.join(reportsDir, jsonFiles[0]),
...
describe("tripp agents review-read", () => {
  it("reads a valid JSON review packet", async () => {
    ...
    await expect(
      executeAgentsReviewRead(reviewPath, tmpDir)
  ...
  it("reads a valid MD review report", async () => {
    ...
    await expect(
      executeAgentsReviewRead(mdPath, tmpDir)
    ).resolves.toBeUndefined();
  });

  it("rejects path traversal", async () => {
    await expect(
      executeAgentsReviewRead("/etc/passwd", tmpDir)
    ).rejects.toThrow(/traversal/);
  });
  ...
  it("rejects malformed JSON review packet", async () => {
    ...
    await expect(
      executeAgentsReviewRead(badFile, tmpDir)
    ).resolves.toBeUndefined();
  });
});
...
import {
  executeAgentsTraceAppend,
  executeAgentsTraceList,
  executeAgentsTraceValidate,
  executeAgentsTraceChain,
} from "../agentsCommand.js";
import {
  ensureTraceLedger,
  createTraceEvent,
...
describe("tripp agents trace list", () => {
  it("lists trace events", async () => {
    ...
    // Should not throw
    await expect(
      executeAgentsTraceList({ workdir: tmpDir })
    ).resolves.toBeUndefined();
  });
  ...
  it("filters by packetId", async () => {
    ...
    await expect(
      executeAgentsTraceList({ packetId: "p-a", workdir: tmpDir })
    ).resolves.toBeUndefined();
  });
});
...
describe("tripp agents trace validate", () => {
  it("validates a clean ledger", async () => {
    ...
    await expect(
      executeAgentsTraceValidate(tmpDir)
    ).resolves.toBeUndefined();
  });
...
describe("tripp agents trace chain", () => {
  it("shows causal chain", async () => {
    ...
    await expect(
      executeAgentsTraceChain(linked.eventId, tmpDir)
    ).resolves.toBeUndefined();
  });

  it("handles unknown event gracefully", async () => {
    await expect(
      executeAgentsTraceChain("nonexistent-id", tmpDir)
    ).resolves.toBeUndefined();
  });
});
