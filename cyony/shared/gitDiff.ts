...
export const gitDiffTool: Tool = {
  name: "git_diff",
  description:
    "Read-only git diff: returns --stat summary by default (set statOnly=false for full diff with a path, no approval required).",
  ...
  async execute(input: unknown, context: ToolContext): Promise<ToolResult> {
    ...
    if (path) {
      ...
      if (normalized === wd) {
        ...
        return toolError(
          `Path '${path}' resolved outside workdir: ${normalized}`
        );
      }
    }
