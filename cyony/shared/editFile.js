 ...
 */
async function createBackup(filePath, workdir) {
...
export const editFileTool = {
    name: "edit_file",
    description: "Replace expected text with replacement in a file. Requires approval. Workdir-bound. Backup before modify. Fails if expected not found.",
    ...
    async execute(input, context) {
        ...
        const { path: inputPath, expected, replacement, replaceAll, allowWholeFileReplace, } = parsed.data;
        ...
        if (occurrences === 0) {
            return executionError("edit_file", `Expected text not found in ${inputPath}. No changes made.`);
        }
        // 4. Whole-file replacement guard
        if (!allowWholeFileReplace && expected === existingContent) {
            return executionError("edit_file", `Expected text is the entire file. Set allowWholeFileReplace=true to proceed.`);
        }
        ...
        try {
            ...
            return executionError("edit_file", `Backup failed for ${inputPath}: ${err instanceof Error ? err.message : String(err)}. Aborting edit.`);
        }
        ...
        try {
            ...
            return executionError("edit_file", `Failed to write edited file: ${err instanceof Error ? err.message : String(err)}`);
        }
