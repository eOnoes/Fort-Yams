...
export const writeFileTool = {
    name: "write_file",
    description: "Write content to a file. Requires approval. Workdir-bound. Backup before overwrite. No silent overwrite.",
    ...
    async execute(input, context) {
        ...
        if (existedBefore && !overwrite) {
            return executionError("write_file", `File already exists: ${inputPath}. Set overwrite=true to replace it.`);
        }
        ...
        if (!existedBefore) {
            ...
            try {
                ...
                if (!createParents) {
                    return executionError("write_file", `Parent directory does not exist: ${dirname(inputPath)}. Set createParents=true to create it.`);
                }
        ...
        if (existedBefore) {
            try {
                ...
                return executionError("write_file", `Backup failed for ${inputPath}: ${err instanceof Error ? err.message : String(err)}. Aborting write.`);
            }
        }
        // 6. Write the file
        try {
            ...
            return executionError("write_file", `Failed to write file: ${err instanceof Error ? err.message : String(err)}`);
        }
        // 7. Get new size
        let sizeAfter;
        try {
