...
const SMALL_ROLES: WorkerRole[] = ["researcher", "coder", "tester"];
 ...
 */
export function createPlan(input: PlannerInput): SwarmRunPlan {
  ...
  // Create subagents
  const subagents: SubagentSpec[] = roles.map((role, i) => ({
  ...
  // Create task packets
  const taskPackets: TaskPacket[] = roles.map((role, i) => ({
  ...
  const assignments: SubagentAssignment[] = roles.map((_, i) => ({
    id: createId("asgn"),
...
function getDefaultTier(role: WorkerRole): ModelTierLabel {
  switch (role) {
    case "planner":
    case "warden":
...
function buildSystemPrompt(role: WorkerRole, prompt: string): string {
  switch (role) {
    case "researcher":
...
function buildObjective(role: WorkerRole, prompt: string): string {
