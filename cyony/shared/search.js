...
export const searchTool = {
    name: "search",
    description: "Search for text in files under a path. Case-insensitive substring match. Returns file, line, column, and preview.",
    ...
    async execute(input, context) {
        ...
        // Search for matches
        try {
            const results = await searchDirectory(resolvedPath, queryLower, maxResults, includeExtensions, context.workdir);
            return {
                status: "ok",
                output: {
                    query,
 ...
 */
async function searchDirectory(dirPath, queryLower, maxResults, includeExtensions, workdir) {
    const results = [];
    await searchRecursive(dirPath, queryLower, maxResults, includeExtensions, workdir, results);
    return results;
}
async function searchRecursive(dirPath, queryLower, maxResults, includeExtensions, workdir, results) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            // Recurse
            await searchRecursive(fullPath, queryLower, maxResults, includeExtensions, workdir, results);
        }
            ...
            try {
                const fileResults = await searchFile(fullPath, queryLower, maxResults - results.length, workdir);
 ...
 */
async function searchFile(filePath, queryLower, maxResults, workdir) {
    const results = [];
    // Check file size
