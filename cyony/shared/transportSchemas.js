 ...
 */
import { z } from "zod";
import { ExternalAgentRoleSchema, ExternalAgentTaskPacketSchema, ExternalAgentResultPacketSchema, } from "./schemas.js";
...
// ── Runtime validators ───────────────────────────────────────────────
function transportSafetyRules(data, ctx) {
    // allowDirectMutation must always be false
    if (data.allowDirectMutation) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "allowDirectMutation must always be false — all mutations flow through ApprovalGate",
...
export const ValidatedTransportConfigSchema = ExternalAgentTransportConfigSchema.superRefine(transportSafetyRules);
...
// ── Runtime validators ───────────────────────────────────────────────
export const ValidatedDispatchResultSchema = ExternalAgentDispatchResultSchema.superRefine((data, ctx) => {
    // completed/fake_completed with resultPacket must validate
    if ((data.status === "completed" || data.status === "fake_completed") &&
        data.resultPacket) {
        const parsed = ExternalAgentResultPacketSchema.safeParse(data.resultPacket);
        if (!parsed.success) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
...
//# sourceMappingURL=transportSchemas.js.map