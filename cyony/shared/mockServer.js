...
import { createResponse, createErrorResponse, serializeMessage, parseMessage, isRequest, JSON_RPC_ERRORS, } from "./jsonRpc.js";
// ── Mock Server Info ────────────────────────────────────────────
const SERVER_INFO = {
    name: "mock-echo",
    version: "1.0.0",
...
const MOCK_TOOLS = [
    {
        name: "mock_echo",
        description: "Echo back the input. Safe read-only tool for testing. Non-mutating.",
        inputSchema: {
            type: "object",
            properties: {
                message: {
    ...
    {
        name: "mock_mutate",
        description: "Mock destructive operation. Requires approval. Does not actually mutate files.",
        inputSchema: {
            type: "object",
            properties: {
                target: {
...
function handleToolCall(request) {
    ...
    if (!tool) {
        return serializeMessage(createErrorResponse(request.id, JSON_RPC_ERRORS.METHOD_NOT_FOUND, `Tool not found: ${toolName ?? "unknown"}`));
    }
    ...
    switch (toolName) {
        ...
        default:
            return serializeMessage(createErrorResponse(request.id, JSON_RPC_ERRORS.INTERNAL_ERROR, `Tool '${toolName}' has no mock implementation`));
    }
}
function handleShutdown(request) {
    return serializeMessage(createResponse(request.id, {}));
}
function handleMethodNotFound(request) {
    return serializeMessage(createErrorResponse(request.id, JSON_RPC_ERRORS.METHOD_NOT_FOUND, `Method not found: ${request.method}`));
}
// ── Message Processing ──────────────────────────────────────────
function processLine(line) {
    try {
        ...
        return serializeMessage(createErrorResponse(null, JSON_RPC_ERRORS.PARSE_ERROR, errorMsg));
    }
}
// ── Main ────────────────────────────────────────────────────────
/**
...
export function runMockServer() {
    ...
    process.on("uncaughtException", (err) => {
        process.stderr.write(`Mock MCP server error: ${err.message}\n`);
        process.exit(1);
    });
}
// When run directly (not imported), start the server
// Check if this is the main module by comparing argv
const isMainModule = process.argv[1]?.includes("mockServer") ?? false;
if (isMainModule) {
    runMockServer();
}
