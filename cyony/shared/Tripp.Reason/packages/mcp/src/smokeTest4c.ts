...
import type { Tool, ToolContext } from "@tripp-reason/shared";
...
try {
  ...
  assert(
    echoAdapter.name === "mcp.mock-echo.mock_echo",
    `5. Echo adapter namespaced: ${echoAdapter.name}`,
  );
  assert(
    mutateAdapter.name === "mcp.mock-echo.mock_mutate",
    `5b. Mutate adapter namespaced: ${mutateAdapter.name}`,
  );
  ...
  const echoOutput = echoResult.output as { echoed: string };
  assert(
    echoOutput?.echoed === "hello",
    `10b. Echo echoed 'hello' (got '${echoOutput?.echoed}')`,
  );
  console.log("  ✅ Echo returned:", JSON.stringify(echoOutput));

  // Test 11: mutate execute
  console.log("\n--- Test 11: Mutate execution ---");
  const mutateResult = await mutateAdapter.execute(
    { target: "/tmp/test", action: "create" },
    mockCtx,
  );
  assert(mutateResult.status === "ok", `11a. Mutate status=ok`);
  const mutateOutput = mutateResult.output as Record<string, unknown>;
  assert(mutateOutput?.accepted === true, "11b. Mutate accepted=true");
  assert(
    typeof mutateOutput?.note === "string" && mutateOutput.note.includes("mock only"),
    "11c. Mutate note confirms mock-only",
  );
