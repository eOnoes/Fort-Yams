 ...
 */
import { startServer, type ServerConfig } from "@tripp-reason/server";

export async function executeServe(options: {
  host?: string; port?: string; workdir?: string; db?: string;
  baseUrl?: string; apiKeyEnv?: string; model?: string; providerName?: string;
}): Promise<void> {
