...
// ── Type adapters: Drizzle row → shared type ────────────────────────
// Drizzle rows from better-sqlite3 + snake_case property names match
// shared shapes almost exactly, EXCEPT JSON TEXT columns which we
// need to parse. These helpers translate one row at a time.

type SessionRow = typeof schema.sessions.$inferSelect;
type RunRow = typeof schema.runs.$inferSelect;
type MessageRow = typeof schema.messages.$inferSelect;
type EventRow = typeof schema.events.$inferSelect;
type ToolCallRow = typeof schema.tool_calls.$inferSelect;
type ApprovalRow = typeof schema.approvals.$inferSelect;
type ReportRecordRow = typeof schema.reports.$inferSelect;

function sessionRowToShared(row: SessionRow): Session {
  return {
    id: row.id,
    title: row.title,
...
function runRowToShared(row: RunRow): Run {
  return {
    id: row.id,
...
function messageRowToShared(row: MessageRow): Message {
  return {
    id: row.id,
    session_id: row.session_id,
    run_id: row.run_id,
...
function eventRowToShared(row: EventRow): Event {
  return {
    id: row.id,
    session_id: row.session_id,
    run_id: row.run_id,
...
function toolCallRowToShared(row: ToolCallRow): ToolCall {
  return {
    id: row.id,
    session_id: row.session_id,
    run_id: row.run_id,
...
function approvalRowToShared(row: ApprovalRow): ApprovalRecord {
  return {
    id: row.id,
    session_id: row.session_id,
    run_id: row.run_id,
...
function reportRowToShared(row: ReportRecordRow): ReportRecord {
  return { ...row };
}
 ...
 */
export function createRepositories(db: TrippDb) {
  // ── Session repository ────────────────────────────────────────────
  async function createSession(input: Session): Promise<Session> {
  ...
  // ── Run repository ────────────────────────────────────────────────
  async function createRun(input: Run): Promise<Run> {
  ...
  // ── Message repository ────────────────────────────────────────────
  async function createMessage(input: Message): Promise<Message> {
  ...
  async function listMessagesBySession(sessionId: string): Promise<Message[]> {
    const rows = db
      .select()
  ...
  async function listMessagesByRun(runId: string): Promise<Message[]> {
    const rows = db
      .select()
      .from(schema.messages)
  ...
  // ── Event repository ──────────────────────────────────────────────
  async function createEvent(input: Event): Promise<Event> {
  ...
  async function listEventsByRun(runId: string): Promise<Event[]> {
    const rows = db
      .select()
      .from(schema.events)
  ...
  // ── ToolCall repository ───────────────────────────────────────────
  async function createToolCall(input: ToolCall): Promise<ToolCall> {
  ...
  async function listToolCallsByRun(runId: string): Promise<ToolCall[]> {
    const rows = db
      .select()
      .from(schema.tool_calls)
  ...
  // ── Approval repository ───────────────────────────────────────────
  async function createApproval(input: ApprovalRecord): Promise<ApprovalRecord> {
  ...
  async function listApprovalsByRun(runId: string): Promise<ApprovalRecord[]> {
    const rows = db
      .select()
      .from(schema.approvals)
  ...
  // ── Report repository ─────────────────────────────────────────────
  async function createReportRecord(input: ReportRecord): Promise<ReportRecord> {
  ...
  async function getReportByRun(runId: string): Promise<ReportRecord | null> {
    const row = db
      .select()
      .from(schema.reports)
  ...
  async function listReportsBySession(sessionId: string): Promise<ReportRecord[]> {
    const rows = db
      .select()
      .from(schema.reports)
