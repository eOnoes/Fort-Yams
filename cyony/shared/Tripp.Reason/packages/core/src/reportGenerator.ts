...
function extractFilesChanged(toolCalls: ToolCall[]): string[] {
  const files = new Set<string>();
  for (const tc of toolCalls) {
    if (tc.result && typeof tc.result === "object" && tc.result !== null) {
      const r = tc.result as Record<string, unknown>;
      if (typeof r.path === "string") files.add(r.path);
      if (typeof r.file === "string") files.add(r.file);
      // Phase 2E: some tools nest path under output (ToolResult shape)
      const output = r.output;
      if (output && typeof output === "object") {
        const o = output as Record<string, unknown>;
        if (typeof o.path === "string") files.add(o.path);
        if (typeof o.file === "string") files.add(o.file);
      }
    }
  }
  return [...files].sort();
...
function renderMarkdown(r: RunReport): string {
  ...
  if (r.persistenceWarnings && r.persistenceWarnings.length > 0) {
    ...
    lines.push("");
    lines.push(
      `> ⚠️ ${r.persistenceWarnings.length} persistence operation(s) failed. Audit trail is incomplete.`
    );
    lines.push("");
