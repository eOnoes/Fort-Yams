/**
 * @tripp-reason/external-agents — Trace Ledger Schemas
 *
...
function validateEventTypeRules(
  ...
  if (
    (data.eventType === "tools_loaded" || data.eventType === "tools_unloaded") &&
  ...
  if (
    data.eventType === "warden_stop_resolved" &&
    !data.parentEventId &&
    ...
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message:
        "warden_stop_resolved must include parentEventId or resolution details",
      path: ["parentEventId"],
    });
  }
  ...
  if (
    data.eventType === "validation_failed_later" &&
    !data.parentEventId &&
    ...
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message:
        "validation_failed_later should include parentEventId or rootCauseEventId when known",
      path: ["parentEventId"],
    });
  }
  ...
  if (
    data.eventType === "approval_timeout" &&
    !data.packetId &&
    !data.reviewId &&
    !data.runId
  ) {
...
export const ValidatedTraceEventSchema =
  AgentBusTraceEventSchema.superRefine(validateEventTypeRules);
...
export type TraceLedgerValidationResult = z.infer<
  typeof TraceLedgerValidationResultSchema
>;
