...
describe("trace ledger helpers", () => {
  ...
  // ── Stage 3A: Timeout trace events ──────────────────────────────

  it("task_timeout validates with packetId", () => {
    const event = createTraceEvent({
      eventType: "task_timeout",
      severity: "error",
      actorType: "system",
      packetId: randomUUID(),
      summary: "Worker task timed out after 300s",
      details: { timeoutMs: 300000, agentRole: "hermes_cyony" },
    });
    expect(event.eventType).toBe("task_timeout");
    expect(event.packetId).toBeDefined();
    expect(event.severity).toBe("error");
  });

  it("task_timeout validates with subagentId", () => {
    const event = createTraceEvent({
      eventType: "task_timeout",
      severity: "error",
      actorType: "system",
      subagentId: "worker-1",
      summary: "Subagent worker timed out",
    });
    expect(event.eventType).toBe("task_timeout");
    expect(event.subagentId).toBe("worker-1");
  });

  it("task_timeout rejects missing packetId and subagentId", () => {
    expect(() =>
      createTraceEvent({
        eventType: "task_timeout",
        actorType: "system",
        summary: "Timeout with no causal link",
      })
    ).toThrow();
  });

  it("tool_timeout validates with toolNames", () => {
    const event = createTraceEvent({
      eventType: "tool_timeout",
      severity: "warning",
      actorType: "approvalgate",
      toolNames: ["shell"],
      summary: "Tool shell timed out after 30s",
      details: { timeoutMs: 30000 },
    });
    expect(event.eventType).toBe("tool_timeout");
    expect(event.toolNames).toContain("shell");
  });

  it("tool_timeout rejects empty toolNames", () => {
    expect(() =>
      createTraceEvent({
        eventType: "tool_timeout",
        actorType: "system",
        summary: "Tool timeout with no tools specified",
      })
    ).toThrow();
  });

  it("approval_timeout validates with packetId", () => {
    const event = createTraceEvent({
      eventType: "approval_timeout",
      severity: "warning",
      actorType: "approvalgate",
      packetId: randomUUID(),
      summary: "Approval timed out after 5 minutes",
      details: { timeoutMs: 300000 },
    });
    expect(event.eventType).toBe("approval_timeout");
    expect(event.packetId).toBeDefined();
  });

  it("approval_timeout validates with reviewId", () => {
    const event = createTraceEvent({
      eventType: "approval_timeout",
      severity: "warning",
      actorType: "approvalgate",
      reviewId: randomUUID(),
      summary: "Review approval timed out",
    });
    expect(event.eventType).toBe("approval_timeout");
  ...
  it("approval_timeout rejects missing packetId, reviewId, and runId", () => {
    expect(() =>
      createTraceEvent({
        eventType: "approval_timeout",
        actorType: "system",
        summary: "Approval timeout with no causal link",
      })
    ).toThrow();
  });

  it("timeout events append and read back from ledger", async () => {
    await ensureTraceLedger(tmpDir);
    const pid = randomUUID();
    const event = await appendTraceEvent(
      createTraceEvent({
        eventType: "task_timeout",
        severity: "error",
        actorType: "system",
        packetId: pid,
        summary: "Timeout persisted to ledger",
      }),
      tmpDir
    );
    expect(event.packetId).toBe(pid);

    const events = await readTraceEvents({ workdir: tmpDir });
    const timeoutEvents = events.filter((e) => e.eventType === "task_timeout");
    expect(timeoutEvents.length).toBe(1);
    expect(timeoutEvents[0].packetId).toBe(pid);
  });

  it("timeout events do not imply mutation", () => {
    const event = createTraceEvent({
      eventType: "task_timeout",
      actorType: "system",
      packetId: randomUUID(),
      summary: "Timeout — no mutation occurred",
    });
    // Timeout events are info/warning/error severity — never mutation-related
    expect(event.eventType).not.toBe("mutation_applied");
    expect(event.eventType).not.toBe("mutation_requested");
    // No mutation flags in details
    expect(event.details?.mutated).toBeUndefined();
  });
});
