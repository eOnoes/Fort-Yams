...
const ALLOWED_STATUS_ARGS = ["status", "--short", "--branch"] as const;
...
function parseGitStatusOutput(raw: string): {
  branchLine?: string;
  entries: string[];
} {
...
export const gitStatusTool: Tool = {
  name: "git_status",
  description:
    "Read-only git status: returns branch info and file state summary (no approval required, read-only).",
  ...
  async execute(input: unknown, context: ToolContext): Promise<ToolResult> {
    ...
    try {
      ...
      const exitCode: number =
        typeof e.code === "number" ? (e.code as number) : -1;
      ...
      return executionError(
        "git_status",
        `git exited with code ${exitCode}: ${stderrSnippet || "(no stderr)"}`
      );
    }
  },
};
