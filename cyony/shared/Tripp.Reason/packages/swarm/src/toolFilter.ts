 ...
 */
function toolMatches(name: string, list: string[]): boolean {
  return list.some((entry) => {
    // Exact match
    if (entry === name) return true;
    // Namespace prefix match: "mcp.*" matches all MCP tools
    if (entry === "mcp.*" && name.startsWith("mcp.")) return true;
...
export function filterTools(
  ...
  return tools.filter((tool) => {
    ...
    if (forbiddenTools && forbiddenTools.length > 0) {
      if (toolMatches(name, forbiddenTools)) return false;
    }
    // Check allowlist (if defined)
    if (allowedTools && allowedTools.length > 0) {
      if (!toolMatches(name, allowedTools)) return false;
    }
    return true;
  });
}
 ...
 */
export function createFilteredDispatcher(
