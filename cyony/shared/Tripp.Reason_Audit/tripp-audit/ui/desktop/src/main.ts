...
const MENU_TRANSLATIONS_ZH_CN: Record<string, string> = {
  ...
  'Focus Tripp Window': '聚焦 Tripp 窗口',
  'Quick Launcher': '快速启动器',
  'Always on Top': '窗口置顶',
  'Toggle Navigation': '切换导航',
  'About Tripp': '关于 Tripp',
      ...
      if (protocolUrl) {
        ...
        if (parsedUrl.hostname === 'bot' || parsedUrl.hostname === 'recipe') {
          app.whenReady().then(async () => {
            ...
            if (deeplinkData) {
              const confirmed = await confirmRecipeDeepLink(protocolUrl, deeplinkData.config);
              if (!confirmed) {
                log.warn('[Main] Recipe deep link rejected by user via second-instance:', protocolUrl);
                return;
              }
            }
...
async function processProtocolUrl(url: string, parsedUrl: URL, window: BrowserWindow) {
  ...
  if (parsedUrl.hostname === 'extension') {
    ...
    if (deeplinkData) {
      const confirmed = await confirmRecipeDeepLink(url, deeplinkData.config);
      if (!confirmed) {
        log.warn('[Main] Recipe deep link rejected by user:', url);
        return;
      }
    }
    const scheduledJobId = parsedUrl.searchParams.get('scheduledJob');
...
const windowWorkingDirs = new Map<number, string>(); // windowId -> workingDir
...
function getAllowedRootsForWindow(windowId: number): string[] {
  const roots: string[] = [];
  const userData = app.getPath('userData');
  roots.push(userData);

  const workingDir = windowWorkingDirs.get(windowId);
  if (workingDir) {
...
function isPathWithinRoot(target: string, root: string): boolean {
  ...
  // Ensure the root ends with a separator so /foo isn't a parent of /foobar
...
function isPathAllowed(targetPath: string, windowId: number): boolean {
  const expanded = expandTilde(targetPath);
  const resolved = canonicalizePath(expanded);

  // Deny paths that contain traversal sequences after expansion
  if (resolved.includes('..')) {
    return false;
  }

  // Deny symlinks by checking if the real path differs (best-effort)
  try {
    const real = fsSync.realpathSync(resolved);
    if (real !== resolved) {
      // Allow if the real path still resolves within an allowed root
      const roots = getAllowedRootsForWindow(windowId);
      const stillAllowed = roots.some((root) => isPathWithinRoot(real, root));
      if (!stillAllowed) {
        return false;
      }
    }
  } catch {
    // If realpath fails, proceed with path-based checks only
  }
...
// ── Recipe Deep Link Security ─────────────────────────────────────────

const HIGH_RISK_RECIPE_TOOLS = new Set([
  'shell',
  'developer',
  'computer_control',
  'computer-control',
...
function parseRecipeConfig(configStr: string): { tools: string[]; workingDir?: string } {
  const tools: string[] = [];
  ...
  try {
    ...
    if (typeof parsed.working_dir === 'string') {
      workingDir = parsed.working_dir;
    }
  } catch {
    // If parsing fails, return empty
  }
  return { tools, workingDir };
}

function hasHighRiskRecipeTools(tools: string[]): boolean {
  return tools.some((t) => HIGH_RISK_RECIPE_TOOLS.has(t.toLowerCase()));
}

async function confirmRecipeDeepLink(
  sourceUrl: string,
  configStr: string
): Promise<boolean> {
  const { tools, workingDir } = parseRecipeConfig(configStr);
  const hasHighRisk = hasHighRiskRecipeTools(tools);

  const detailParts = [
    `Source URL: ${sourceUrl}`,
    workingDir ? `Working directory: ${workingDir}` : '',
    tools.length > 0 ? `Requested tools: ${tools.join(', ')}` : 'No tools specified',
    hasHighRisk
      ? '⚠️ WARNING: This recipe requests high-risk tools (shell/developer/computer-control).'
      : '',
    'Only proceed if you trust the source.',
  ].filter(Boolean);

  const result = await dialog.showMessageBox({
    type: hasHighRisk ? 'warning' : 'question',
    title: 'Confirm Recipe Import',
    message: 'A recipe wants to configure Tripp. Do you want to proceed?',
    detail: detailParts.join('\n\n'),
    buttons: hasHighRisk ? ['Cancel', 'Proceed (Not Recommended)'] : ['Cancel', 'Proceed'],
    defaultId: 0,
    cancelId: 0,
  });
  ...
  if (process.platform !== 'win32') {
    ...
    if (parsedUrl.hostname === 'bot' || parsedUrl.hostname === 'recipe') {
      ...
      if (deeplinkData) {
        const confirmed = await confirmRecipeDeepLink(url, deeplinkData.config);
        if (!confirmed) {
          log.warn('[Main] Recipe deep link rejected by user via open-url:', url);
          return;
        }
...
ipcMain.handle('get-secret-key', () => {
  // SECURITY NOTE: Returning the full server secret to the renderer exposes it to
  // any XSS or compromised renderer content. The renderer currently needs this to
  // authenticate direct HTTP calls to the local goosed backend. A future refactor
  // should move these calls to main-process-proxied IPC or use scoped/session tokens.
  const settings = getSettings();
  const secret = getServerSecret(settings);
  log.warn('[Security] get-secret-key invoked — secret exposed to renderer process');
  return secret;
});
...
ipcMain.handle('get-acp-url', async (event) => {
  ...
  // SECURITY: Do not include the long-lived secret in the URL returned to the
  // renderer. Query parameters end up in browser history, logs, and referrer
  // headers. The backend (check_acp_token) already removed query-param support
  // and expects X-Secret-Key in headers. The renderer should negotiate auth
  // via a separate scoped-token handshake or main-process proxy.
...
ipcMain.handle('read-file', async (event, filePath) => {
  try {
    ...
    const windowId = BrowserWindow.fromWebContents(event.sender)?.id;
    if (windowId !== undefined && !isPathAllowed(expandedPath, windowId)) {
      console.error(`[Main] read-file denied: path outside allowed roots: ${expandedPath}`);
      return { file: '', filePath: expandedPath, error: 'Path not allowed', found: false };
    }
    if (process.platform === 'win32') {
...
ipcMain.handle('write-file', async (event, filePath, content) => {
  try {
    ...
    const windowId = BrowserWindow.fromWebContents(event.sender)?.id;
    if (windowId !== undefined && !isPathAllowed(expandedPath, windowId)) {
      console.error(`[Main] write-file denied: path outside allowed roots: ${expandedPath}`);
      return false;
    }
...
ipcMain.handle('ensure-directory', async (event, dirPath) => {
  try {
    ...
    const windowId = BrowserWindow.fromWebContents(event.sender)?.id;
    if (windowId !== undefined && !isPathAllowed(expandedPath, windowId)) {
      console.error(`[Main] ensure-directory denied: path outside allowed roots: ${expandedPath}`);
      return false;
    }
...
ipcMain.handle('list-files', async (event, dirPath, extension) => {
  try {
    ...
    const windowId = BrowserWindow.fromWebContents(event.sender)?.id;
    if (windowId !== undefined && !isPathAllowed(expandedPath, windowId)) {
      console.error(`[Main] list-files denied: path outside allowed roots: ${expandedPath}`);
      return [];
    }
...
async function appMain() {
  ...
  const appMenu = menu?.items.find((item) => item.label === 'Tripp');
  if (appMenu?.submenu) {
  ...
  if (fileMenu?.submenu) {
    ...
    if (shortcuts.focusWindow) {
      fileMenu.submenu.append(
        new MenuItem({
          label: menuT('Focus Tripp Window'),
  ...
  if (menu && process.platform !== 'darwin') {
    ...
    if (helpMenu.submenu) {
      ...
      // Create the About Tripp menu item with a submenu
      const aboutGooseMenuItem = new MenuItem({
        label: menuT('About Tripp'),
        submenu: Menu.buildFromTemplate([]), // Start with an empty submenu for About
      });

      // Add the Version menu item (display only) to the About Tripp submenu
...
app.whenReady().then(async () => {
  try {
    ...
    dialog.showErrorBox('Tripp Error', `Failed to create main window: ${error}`);
    app.quit();
  }
});
