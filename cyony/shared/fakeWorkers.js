...
function isSimulated(ctx) {
    return ctx.simulateTimeout === true || ctx.simulateFail === true;
}
function makeTimeoutResult(task, ctx) {
    const r = baseResult(task, ctx);
    r.status = "partial";
...
function makeFailResult(task, ctx) {
    const r = baseResult(task, ctx);
    r.status = "fail";
 ...
 */
export function runFakeWorker(subagent, task, ctx) {
...
//# sourceMappingURL=fakeWorkers.js.map