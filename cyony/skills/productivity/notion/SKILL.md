...
### Create a database (v2025-09-03 — TWO-STEP, CRITICAL)

In API version 2025-09-03, `POST /v1/databases` creates the container but **silently drops all inline properties except the title**. You must add schema properties in a separate `PATCH` call against the data source.
...
# Response has no "properties" field. Look for "data_sources"[0]["id"] — that is the data_source_id.
```

**Step 2 — Add schema properties to the data source:**
```bash
...
# Verify: count properties in response.
```

**Pitfall:** `POST /v1/data_sources` returns `"Creating new databases with data sources is not supported in this endpoint for API version 2025-09-03 and later"`. Always use `POST /v1/databases` then `PATCH /v1/data_sources/{id}`.
**Pitfall:** If you create a page with `parent: {"database_id": "..."}` and include property values for fields that don't exist on the data source yet, you get `"X is not a property that exists"`. Always add properties to the data source BEFORE creating rows.
**Pitfall:** Internal integrations cannot create workspace-level pages (`"parent": {"type": "workspace"}` returns validation error). Always create pages under an existing page_id that has been shared with the integration.
...
- **Agent coordination pattern:** see `references/agent-coordination-pattern.md` for the Build Queue + Dashboard workspace used by the Cyony/Tripp/Echo/Codex crew.
