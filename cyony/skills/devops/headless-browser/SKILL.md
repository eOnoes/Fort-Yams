...
- **Tailscale Serve for HTTPS** — when you need HTTPS (e.g., PWA install prompts, WebAuthn) but don't want a public tunnel, use `tailscale serve`. **Prerequisite:** Tailscale Serve must be enabled in the admin panel first (`https://login.tailscale.com/admin/serve`). Without this, `tailscale serve` fails with a misleading error. Once enabled: `tailscale serve --bg --set-path / http://localhost:PORT`. Gives HTTPS on the tailnet (e.g., `https://100.x.x.x`).
- **PWA install requires HTTPS** — Chrome/Firefox hide the "Install app" prompt entirely on HTTP sites. Even `*.trycloudflare.com` counts as HTTPS for this purpose. If user reports "no install option," check the protocol first.
