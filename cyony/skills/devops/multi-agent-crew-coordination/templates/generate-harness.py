#!/usr/bin/env python3
"""
TripCore Harness v1.1 Generator — Windows-Safe + HTML-Escaped
Reads current-state.md → generates static HTML dashboard + optional snapshot.json
...
Cross-platform requirements:
  - encoding="utf-8" on ALL file reads/writes (Windows defaults to cp1252)
  - html.escape() ALL Markdown-derived values before HTML interpolation
  - Atomic .tmp writes → os.replace() on success (never zero existing files)
"""
...
def generate_html(meta, sections, agents, lane, priority, evidence, stops, notes):
    E = html.escape  # CRITICAL: all Markdown-derived values MUST be escaped
